class Solution {
    public:
        /* 
        Our main objective here is to find out whether the current num can be placed in any sequence.
    Here for each num in nums first try placing it in one of the existing subseq or if not possible then see if 
    creating a new subseq. of atleast length 3 starting with that num.
